# Story-to-Game · Tully Optimized Edition

一套从「小说/剧本转互动文游」到「运行互动文游」的全流程工具。这个版本重点优化短素材改写效率、示例体验和线上复现路径，包含两大核心组件：
...
本仓库是面向「短素材快速生成可玩分支剧情」整理的优化版本，保留启动器和 AI 技能的基础能力，同时收敛了短素材流程、补齐示例和线上体验入口。MIT 版权声明保留在 `LICENSE` 中。

Tully 在此版本中完成的主要改进：
- 新增「快速模式」绿色通道：针对短素材、生活片段、单场景速写，跳过冗长确认流程并直接落盘 JSON。
- 保留质量底线：快速模式仍强制保留选项回响、callback、closure、多结局和校验流程，避免只做格式转换。
- 实测优化：同类短素材从约 14 分钟的标准流程缩短到约 3 分钟，并产出 50+ 节点、0 错误 0 警告的可玩 JSON。
- 整理线上体验入口：将单文件启动器和示例剧本部署为可直接访问的浏览器体验版，方便快速复现。

本版本重点不是重写叙事引擎，而是把从素材到可玩 JSON 的路径做短、做稳，并把启动器包装成可以直接展示的体验页面。
...
## 许可证
本项目采用 MIT 许可证，保留上游版权声明；详见 [LICENSE] 文件。
